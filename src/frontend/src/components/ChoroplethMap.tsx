  ...
  return (
    <Panel title="District choropleth" subtitle={`${filters.year}`} loading={loading} error={error}>
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1">
        {[
          { color: '#cfe3fd', label: 'Very low accidents' },
          { color: '#86bdfb', label: 'Low accident count' },
          { color: '#4f9cf9', label: 'Medium accident count' },
          { color: '#2d6fb3', label: 'High accident count' },
          { color: '#1e3a5f', label: 'Very high accidents' },
        ].map((item) => (
