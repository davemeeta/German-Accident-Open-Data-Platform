export const SEV_COLOR = { fatal: "#ef4444", serious: "#f59e0b", light: "#319255", all: "#2a68dc" } as const;
...
export function cleanName(name: string | null | undefined): string {
  if (!name) return '';
  return name
    .replace(/,?\s*(kreisfreie Stadt|Landkreis|Kreis|Stadtkreis|Hansestadt|Stadt)\s*/gi, '')
    .trim();
}